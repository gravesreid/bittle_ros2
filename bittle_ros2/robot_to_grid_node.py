...
class CommandPublisher(Node):
    ...
    #    self.current_heading = msg.orientation.x
    #    self.get_logger().info(f'Current heading: {self.current_heading}')

    def get_direction_from_heading(self, heading):
        if 45 <= heading < 135:
            return 'north'
        elif -180 <= heading <= -135 or 135 <= heading <= 180:
            return 'east'
        elif -45 <= heading < 45:
            return 'west'
        elif -45 > heading > -135:
    ...
    def navigate_to_target(self):
        ...
        # Calculate the direction of travel based on the IMU heading
        direction_of_travel = self.get_direction_from_heading(self.current_heading)
        self.get_logger().info(f'Direction of travel: {direction_of_travel}')
